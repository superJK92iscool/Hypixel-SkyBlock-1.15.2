...
public class PetItemRecipe implements IPetRecipe {
	...
	public static class Serializer extends ForgeRegistryEntry<IRecipeSerializer<?>>
			implements IRecipeSerializer<PetItemRecipe> {
		@Override
		public PetItemRecipe read(ResourceLocation recipeId, JsonObject json) {
		...
		@Override
		public PetItemRecipe read(ResourceLocation recipeId, PacketBuffer buffer) {
			final NonNullList<Ingredient> ingredients = NonNullList.withSize(buffer.readVarInt(), Ingredient.EMPTY);
			for (int j = 0; j < ingredients.size(); ++j)
				ingredients.set(j, Ingredient.read(buffer));
			return new PetItemRecipe(recipeId, buffer.readString(0x7FFF), ingredients, buffer.readItemStack());
		}

		@Override
		public void write(PacketBuffer buffer, PetItemRecipe recipe) {
	...
	public PetItemRecipe(@Nonnull ResourceLocation id, @Nonnull String group, @Nonnull NonNullList<Ingredient> inputs,
